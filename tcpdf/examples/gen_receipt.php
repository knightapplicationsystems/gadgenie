...
<tr>        
        ...
        <th style="border:1px solid black;border-collapse:collapse;">Stock Reference</th>
</tr>    
EOD;
    $data = array();
...
//echo "SELECT * FROM _gg_stock WHERE _rec_ref = '$rec'";
    ...
    foreach ($data as $row) {
        $html .='<tr>';
        $html .= '<td style="border:1px solid black;border-collapse:collapse;font-size:14px;">' . $row[0] . '</td>';
        $html .= '<td style="border:1px solid black;border-collapse:collapse;font-size:14px;">£' . $row[1] . '</td>';
        $html .= '<td style="border:1px solid black;border-collapse:collapse;font-size:14px;">£' . $row[2] . '</td>';
        $html .= '<td style="border:1px solid black;border-collapse:collapse;font-size:14px;">' . $row[3] . '</td>';
        $html .= '<td style="border:1px solid black;border-collapse:collapse;font-size:14px;">' . $row[4] . '</td>';
        $html .='<br>';
        $html .='</tr>';
    }
...
// This method has several options, check the source code documentation for more information.
    $pdf->Output($_SERVER['DOCUMENT_ROOT'] . 'api/cust_docs/purchase_receipts/' . $rec . '.pdf', 'F');
//$output = $rec.'pdf';
//$pdf->Output($output, 'I');
//============================================================+
// END OF FILE
